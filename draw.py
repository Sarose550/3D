...
def add_box(matrix, x, y, z, width, height, depth ):
    ...
    add_edge(matrix, x+width, y-height, z, x+width, y-height, z-depth)

    add_edge(matrix, x+width, y-height, z-depth, x, y-height, z-depth)
    add_edge(matrix, x+width, y-height, z-depth, x+width, y, z-depth)

    add_edge(matrix, x, y, z-depth, x, y-height, z-depth)
    add_edge(matrix, x, y, z-depth, x+width, y, z-depth)

def add_sphere(matrix, cx, cy, cz, r):
  theta = 0
  while theta <= 1:
    phi = 0
    while phi <= 1:
        x = r * math.sin(2*math.pi * theta) + cx
        y = r * math.cos(2*math.pi * theta) * math.cos(2*math.pi * phi) + cy
        z = r * math.cos(2*math.pi * theta) * math.sin(2*math.pi * phi) + cz
        add_edge(matrix, x, y, z, x, y, z)
        phi += .02
    theta += .02

def add_torus(matrix, cx, cy, cz, r0, r1):
    theta = 0
    while theta <= 1:
        phi = 0
        while phi <= 1:
            z = (r1 + r0 * math.cos(2 * math.pi * theta)) * math.cos(2 * math.pi * phi) + cz
            x = (r1 + r0 * math.cos(2 * math.pi * theta)) * math.sin(2 * math.pi * phi) + cx
